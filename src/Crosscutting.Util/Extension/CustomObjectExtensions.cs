...
namespace Crosscutting.Util.Extension
{
    public static class CustomObjectExtensions
    {
        ...
        public static DateTime GetServerDateTimeNow(TimeZoneInfo serverTimeZoneInfo)
        {
            return TimeZoneInfo.ConvertTime(DateTime.UtcNow, serverTimeZoneInfo);
        }
        ...
        public static string GetDevExceptionMessage(this Exception ex)
        {


            var stackTrace = new StackTrace(ex, true); // Get stack trace with file info
            var frame = stackTrace.GetFrame(0); // Get the top frame (where the exception occurred)
            ...
            return $"Class:{className},Line:{lineNumber}, Message {ex.Message},InnerException:{InnerException}, trace:{ex.StackTrace}";
